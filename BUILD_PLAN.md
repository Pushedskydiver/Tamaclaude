...
touched again. The one known exception — the boot splash, which lives in the
firmware — was taken in Stage 5 below, on 21 Aug. That was the last flash,
sixteen days before Stage 5, where it was scheduled, even opens.
...
- [x] **The boot splash — design it together, then bake it into the firmware.**
      ...
      **It was also the last thing needing a reflash**, and that flash has
      happened — built clean, verified on the panel, Stage 2's exit is now
      ...
      already there and no new wire format exists to be wrong. 3,456 bytes of
      `.rodata`, 31.9:1.
      `tools/bake-splash.test.ts` is the gate, because the firmware is in none
      of the six and nothing else can be.
