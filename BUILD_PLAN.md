...
- [~] Flash factory demo, confirm display and WS2812 work — **superseded, and
  left open long enough to keep regenerating the question "should we do the
  stragglers first?".** Its two jobs were answering the flash question and
  confirming the board is good. The first is answered on the line above —
  8MB, measured. The second is done better by flashing the blitter, which
  exercises the display through the code that actually ships.
  WS2812 ships unused: `blitter/main/main.c` declares six pins — MOSI 6,
  SCLK 7, CS 14, DC 15, RST 21, BL 22 — and GPIO8 is not among them, nor
  does either firmware pull in `driver/rmt` or a LED-strip component.
  A new board arrives with this demo already on it, so **powering the gift
  board once before flashing gives the whole smoke test free, in a minute**
  — and flashing destroys it, so that is the only moment it is available.
  The image is backed up at `../tamaclaude-factory-backup/factory-4mb.bin`
  (20 Aug) if it is ever wanted back; being a 4 MB dump of an 8MB part, it
...
- [x] Birthday screen, date-triggered 23 Sep. **Trigger, art, stage and QR are
      ...
      **What it costs is the pack's birthday quip on those two states**, measured
      ...
      checked 30 Aug, `https://alexclapperton.co.uk/j` returns 200, as does
      `/j/setup`, both by redirect to the `www.` host with a trailing slash
      added. This line said the URL "serves nothing yet — that is the open
