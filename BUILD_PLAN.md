...
| **Mon 25 Aug**      | **DESIGN FREEZE** — screen list, state machine, pack format locked        |
| Sun 31 Aug          | End-to-end pipeline alive with placeholder art                            |
...
- [x] `git init` — done locally, 58 files staged, nothing committed yet
- [ ] Create the GitHub remote `tamaclaude` (public, per Alex) and push
...
## Stage 1 — Renderer + dev harness (Wed 19 – Sun 24 Aug)
...
- [ ] **SPIKE: generate one real animation end-to-end.** Base SVG → LLM → animated SVG →
...
## Stage 3 — Session pipeline (Sun 24 – Sun 31 Aug)
...
- [ ] `PermissionRequest` → "Wansum?" screen
- [ ] `StopFailure` → "Turrrby, Turrrby, Turrrby"
...
## Stage 4 — Art (Sun 24 Aug – Sun 13 Sep, overlapping)

The long pole. Runs in parallel with Stage 3 from week two.

**Time-box: 2 days per animation, hard.** If one isn't landing, ship the placeholder and
move to the next. Come back to stragglers in the buffer. One perfect animation and eight
missing ones is a worse gift than nine decent ones.

- [ ] `clawd-static-base.svg` — canonical geometry, stable element IDs
- [ ] `PLANS.md` — prose spec per animation (action / body mechanics / eyes / effects)
...
- [ ] Playwright SVG→PNG frame renderer; palette quantise; RLE pack
- [ ] Animations, in priority order — ship each as it lands:
  ...
  5. gym (Bash)
  6. **Model 3 pulls up (Stop)** — the payoff, do not cut
  7. sweeping (PreCompact)
  8. dizzy (StopFailure)
  9. wizard (WebSearch/WebFetch)
  10. board game (Agent/subagents)
  11. road bike (long runs)

## Stage 5 — Personalisation (Sun 6 – Sun 13 Sep)

- [ ] `packs/jamie/` (gitignored): palette, quips, logo, Penny
- [ ] Penny sprite from Alex's photos — background prop on idle/asleep, not the mascot
- [ ] Company logo → pixel: SVG → nearest-neighbour → palette quantise (`sharp`)
- [ ] Quips mapped to states, never randomised
- [ ] Rare easter eggs: Avatar idle (1-in-50), "your mum" idle
...
| Risk                                                                                                                                                                                                       | Mitigation                                                                                                                                                        |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Jamie discovers the repo.** A public repo called `tamaclaude` full of Claude Code hooks, appearing on Alex's GitHub, spoils the surprise — Jamie is a tech-savvy AI enthusiast who may well follow Alex. | Repo stays **private until 23 Sep**, then flips public. Costs nothing.                                                                                            |
...
- Semantics of "Beajilpig", "Vaglig", "Burst Pistol" — needed to map them to states
